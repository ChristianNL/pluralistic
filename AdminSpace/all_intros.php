...
<head>
    ...
    <title>Dashboard - All Introductions</title>
                ...
                <div class="card mb-4">
                    ...
                    <div id="abonnes-table" class="card-body">
                        <table id="datatablesSimple">
                            ...
                            $result = $conn -> query("SELECT bold_part, image_path, intro_text, date_enr from intro");
                            if ($result -> num_rows > 0) {
                                while ($row = $result -> fetch_assoc()) {
                                    echo "<tr><td>". $row['bold_part']. "</td><td>". $row['intro_text']. "</td><td>".
                                        $row['image_path']. "</td><td>". $row['date_enr']. "</td>";
                                    echo "<td><button onclick='deleteIntro(\"" . $row['bold_part'] . "\")'>Supprimer</button></td></tr>";
                                }
                                echo "</table>";
                            } else {
                                echo "Aucune Introduction pour le moment";
                            }
                            ...
                            <script>
                                function deleteIntro(boldPart) {
                                    if (confirm("Êtes-vous sûr de vouloir supprimer ceci ?")) {
                                        var xhr = new XMLHttpRequest();
                                        xhr.open("POST", "delete_intro.php", true);
