...
<head>
    ...
    <title>Dashboard - All Raisons</title>
                ...
                <div class="card mb-4">
                    ...
                    <div id="abonnes-table" class="card-body">
                        <table id="datatablesSimple">
                            ...
                            $result = $conn -> query("SELECT raison_title, raison_desc, date_enr from raison");
                            if ($result -> num_rows > 0) {
                                while ($row = $result -> fetch_assoc()) {
                                    echo "<tr><td>". $row['raison_title']. "</td><td>". $row['raison_desc'].
                                        "</td><td>". $row['date_enr']. "</td>";
                                    echo "<td><button onclick='deleteRaison(\"" . $row['raison_title'] . "\")'>Supprimer</button></td></tr>";
                                }
                                echo "</table>";
                            } else {
                                echo "Aucune Raison pour le moment";
                            }
                            ...
                            <script>
                                function deleteRaison(titreRaison) {
                                    if (confirm("Êtes-vous sûr de vouloir supprimer ceci ?")) {
                                        var xhr = new XMLHttpRequest();
                                        xhr.open("POST", "delete_raison.php", true);
                                        ...
                                        xhr.send("titreRaison=" + titreRaison);
                                    }
                                }
                            </script>
                            </tbody>
